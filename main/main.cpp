...
std::optional<ranges::any_view<std::string_view>> findBuiltin(std::string_view cmd) {
  if (cmd.starts_with("iota")) {
    return ranges::views::iota(1) | ranges::views::transform([](auto i) -> std::string_view {
             return "sheep " + std::to_string(i);
           });
  }
  return {};
}
...
struct Command {
  ...
  auto run(ranges::range auto stream) -> ranges::any_view<std::string_view> {
    if (_str.starts_with('{') && _str.ends_with('}')) {
      return ranges::views::single("todo: closure");
...
int main(int argc, char **argv) {
  for (const char *line; (line = linenoise("stream-shell v0.1 🚀> "));) {
    for (auto &&pipeline : parsePipelines(line)) {
      ranges::any_view<std::string_view> stream = ranges::views::empty<std::string_view>;
